import logger from "../../gachon/config/logger.js";
import connection from "../config/db_config.js";
import fs from "fs";
let Csubject={
    getsubject: (res)=>{
        console.log("");
        console.log("C대학 SUBJECT table 정보");
        connection.query('select * from C_SUBJECT',
                         function(err,rows){
                        if(err){
                            console.log(err);
                            logger.log("error","C대학 과목 전송시오류 ERD");
                            res.render("error",{
                                message:'connection Error'
                            });
                        }else{
                            console.log(rows);
                            console.log("C 대학의 subject DB 정보를 송신합니다.");
                            console.log("====================================");
                            // 인터페이스 파일
                            fs.appendFile('cuniv/c.json',JSON.stringify(rows),(err)=>err?console.log(err):false);    

                            logger.log('info','C대학 과목 전송완료 -');
                            res.send(rows);
                        }
                    }
                )
    }
}

export default Csubject;